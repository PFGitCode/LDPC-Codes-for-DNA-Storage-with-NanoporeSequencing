...
function data = method2Soft(llr, H,receivedSignal,mu,matrix)
muA = mu(1:2);
muC = mu(3:4);
muT = mu(5:6);
muG = mu(7:8);
...
for iter = 1:50
    ...
    for i = 1:n/2
        ...
        if (pdfT(i) > pdfA(i) && pdfT(i) > pdfG(i) && receivedSignal(2,i) < muG(2) &&  receivedSignal(1,i) < muA(1))
            ...
            sumi21 = log(((pdfA(i)/(pdfA(i) + pdfC(i)))*Pz0L2+(pdfT(i)/(pdfT(i) + pdfG(i)))*Pz1L2)/((pdfC(i)/((pdfC(i) + pdfG(i))))*Pz0L2+(pdfG(i)/(pdfG(i)+pdfT(i)))*Pz1L2));
            sumi12 = log(((pdfA(i)/(pdfA(i) + pdfT(i)))*Pz0L1+(pdfC(i)/(pdfC(i) + pdfG(i)))*Pz1L1)/((pdfT(i)/((pdfT(i) + pdfG(i))))*Pz0L1+(pdfG(i)/(pdfG(i)+pdfC(i)))*Pz1L1));
            LQ(i) = LQ(i)+sumi21;
