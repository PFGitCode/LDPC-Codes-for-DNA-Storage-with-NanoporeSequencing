...
function [data1,data2] = method1Soft(llr1, llr2, H1, H2,receivedSignal,mu,matrix)
muA = mu(1:2);
muC = mu(3:4);
muT = mu(5:6);
muG = mu(7:8);
...
for iter = 1:50
    ...
    for i = 1:n
        ...
        if (pdfT(i) > pdfA(i) && pdfT(i) > pdfG(i) && receivedSignal(2,i) < muG(2) &&  receivedSignal(1,i) < muA(1))
            LQ1(i) = LQ1(i) - alpha*(sumi2);
            LQ2(i) = LQ2(i) - alpha*(sumi1);
        end
        data1(i) = LQ1(i) < 0;
        data2(i) = LQ2(i) < 0;
    end
    ...
    for i = 1:n
        ...
        if (pdfT(i) > pdfA(i) && pdfT(i) > pdfG(i) && receivedSignal(2,i) < muG(2) &&  receivedSignal(1,i) < muA(1))
            Lq1(nonZerosElementi1,i) = Lq1(nonZerosElementi1,i) - alpha*(sumi2);
            Lq2(nonZerosElementi2,i) = Lq2(nonZerosElementi2,i) - alpha*(sumi1);
        end
    end
end
end
