function [result_pe1, result_pe2, A1, A2] = de_irregular2D_opt(chan1,chan2,iter,ext,mapping,stop_pe,vard,chkd,dv,dc,P1,P2,decodeMethod)
z1 = chan1;
z2 = chan2;
...
while ((c < iter) && ((pe1 > stop_pe)|| (pe2 > stop_pe)))
    ...
    for j=1:dv
        if vardN(j)~=0
            ...
            A1(c,j) = sum(zEx1(1:round((ext(3)-1)/2 + 1)))*ext(2);
            A2(c,j) = sum(zEx2(1:round((ext(3)-1)/2 + 1)))*ext(2);
    ...
    elseif decodeMethod == "method2"
        z1 = method2Ex(xvar_ave1,xvar_aveEX2,ext,P2,2);
        z2 = method2Ex(xvar_ave2,xvar_aveEX1,ext,P1,1);
